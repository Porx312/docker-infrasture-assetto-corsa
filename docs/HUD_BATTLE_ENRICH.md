...
battle poll / WSS push
  → Redis ac:hud:battle:*
  → enrichBattleWithProfiles → peekSessionCache (never Convex)
  → session miss → fallback ac:hud:battle:profile:{steamId} (TTL HUD_BATTLE_PROFILE_TTL_SEC, default 3600s)
  → session peek OK → touch session TTL + persist battle profile cache
  → both miss → snapshot fields from telemetry (AC name, car, score); log source=miss
...
| `HUD_BATTLE_PROFILE_TTL_SEC` | 3600 | Sticky battle opponent profile cache (`ac:hud:battle:profile:{steamId}`) |
...
| `HUD_BATTLE_ENRICH_LOG` | false | Log `[battle-enrich] state=… source=peek\|battle-profile\|snapshot\|miss` |
