...
def load_server_configs(ServerStateClass):
    ...
    def find_cfg_path(base_path):
        candidate = os.path.join(base_path, 'server_cfg.ini')
        if os.path.exists(candidate):
            return candidate
        candidate = os.path.join(base_path, 'cfg', 'server_cfg.ini')
        if os.path.exists(candidate):
            return candidate
        return None
    ...
    for base_path in all_paths:
        cfg_path = find_cfg_path(base_path)
        if not cfg_path:
            print(f"⚠️  server_cfg.ini not found at: {base_path}/server_cfg.ini or {base_path}/cfg/server_cfg.ini")
            continue
