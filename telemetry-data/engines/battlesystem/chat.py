...
from core.hud_sse_presence import has_hud_overlay_connected
...
def notify_player_chat(manager, guid: str, message: str) -> None:
    """Send a battle line to one driver when they have no HUD overlay."""
    if not settings.BATTLE_CHAT_ENABLED or not manager.on_chat_message:
        return
    if has_hud_overlay_connected(guid):
        return
    manager.on_chat_message(guid, message)


def notify_touge_chat(manager, message: str) -> None:
    """Send a battle line to both drivers without an active HUD overlay."""
    if not settings.BATTLE_CHAT_ENABLED or not manager.on_chat_message or not manager.battle:
        return
    for guid in (manager.battle.car1_guid, manager.battle.car2_guid):
        notify_player_chat(manager, guid, message)


def format_matchup(manager) -> str:
    if not manager.battle:
        return "? vs ?"
    g1, g2 = manager.battle.car1_guid, manager.battle.car2_guid
...
def format_arming_countdown(manager, seconds_remaining: int) -> str:
    ...
    return (
        f"{format_matchup(manager)} — BATTLE ARM {seconds_remaining} "
        f"({_arming_conditions_hint()})"
    )


def notify_arming_countdown(manager, seconds_remaining: int) -> None:
    notify_touge_chat(manager, format_arming_countdown(manager, seconds_remaining))


def notify_arming_cancelled(manager) -> None:
    notify_touge_chat(manager, f"{format_matchup(manager)} — BATTLE CANCELLED")
